...
The first job includes guest root disks but **not** host bind mounts such as
...
first backup. The second job protects the bind-mounted app configuration under
`/lake1t/data/cave-server`, including Compose files and `.env` files. It also
...
raw PostgreSQL files are excluded; restore Immich from its SQL export.
...
For application data, restore the relevant directory from
`/seagate4t/backups/critical/latest/cave-server/docker/` after stopping that
service. The logical database dumps are in `databases/`; the Immich export is
in `immich-db/`. Backup directories contain credentials and are intended to be
readable only by the host administrator.
